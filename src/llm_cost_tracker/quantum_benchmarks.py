...
class BenchmarkConfiguration:
    ...
    def __post_init__(self):
        ...
        if self.dependency_densities is None:
            self.dependency_densities = [
                0.1,
                0.3,
                0.5,
            ]  # Probability of dependencies between tasks
...
class QuantumAlgorithmBenchmark:
    ...
    def generate_benchmark_tasks(
        ...
        for i in range(task_count):
            ...
            cpu_req = random.uniform(
                0.1, resource_pool.cpu_cores * params["resource_factor"]
            )
            memory_req = random.uniform(
                0.1, resource_pool.memory_gb * params["resource_factor"]
            )
            ...
            task.probability_amplitude = complex(
                random.uniform(0.7, 1.0), random.uniform(-0.3, 0.3)
            )
    ...
    async def benchmark_quantum_algorithm(
        ...
        # Calculate metrics
        metrics = self._calculate_schedule_metrics(
            schedule, planner.tasks, resource_pool
        )
    ...
    def benchmark_classical_algorithm(
        self, tasks: List[QuantumTask], resource_pool: ResourcePool, algorithm: str
    ) -> BenchmarkResults:
    ...
    def _greedy_priority_schedule(self, tasks: List[QuantumTask]) -> List[str]:
        ...
        return [
            task.id for task in sorted(tasks, key=lambda t: t.priority, reverse=True)
        ]
            ...
            for dep_id in task.dependencies:
                if dep_id in task_dict:
                    ...
                    max_dependency_end = max(
                        max_dependency_end, dep_start + dep_duration
                    )
    ...
    def _round_robin_schedule(
        self, tasks: List[QuantumTask], resource_pool: ResourcePool
    ) -> List[str]:
    ...
    def _calculate_schedule_metrics(
        ...
        avg_resource_util = sum(metrics["resource_utilization"].values()) / len(
        ...
        metrics["quality_score"] = max(
            0.0, avg_resource_util * 40.0 + priority_bonus * 6.0 - dependency_penalty
        )

        return metrics

    def _calculate_quantum_metrics(
        self, planner: QuantumTaskPlanner, schedule: List[str]
    ) -> Dict[str, Any]:
        ...
        for task_id in schedule:
            ...
            if task.entangled_tasks:
                entanglement_utilization += entangled_in_schedule / len(
                    task.entangled_tasks
                )
    ...
    async def run_comprehensive_benchmark(self) -> Dict[str, Any]:
            ...
            for complexity in self.config.complexity_levels:
                for resources in self.config.resource_constraints:
                    ...
                    for dependency_density in self.config.dependency_densities:
                        logger.info(
                            f"Testing configuration: {task_count} tasks, {complexity} complexity, "
                        ...
                        quantum_results = []
                        classical_results = {
                            alg: [] for alg in all_results["classical_results"]
                        }
                        ...
                        for iteration in range(self.config.iterations_per_config):
                            ...
                            try:
                                quantum_result = await self.benchmark_quantum_algorithm(
                                    tasks,
                                    resource_pool,
                                    self.config.max_algorithm_iterations,
                                )
                                quantum_results.append(quantum_result)

                                logger.debug(
                                    f"Quantum result ({experiment_count}/{total_experiments}): "
                            ...
                            for alg_name in classical_results:
                                try:
                                    classical_result = (
                                        self.benchmark_classical_algorithm(
                                    ...
                                    logger.error(
                                        f"Classical benchmark {alg_name} failed: {e}"
                                    )

                        # Store results for this configuration
                        config_key = (
                            f"{task_count}_{complexity}_{dependency_density:.1f}"
                        )
                        ...
                        for alg_name, results in classical_results.items():
                            if results:
                                all_results["classical_results"][alg_name].extend(
                                    results
                                )

        # Calculate summary statistics
        all_results["summary_statistics"] = self._calculate_summary_statistics(
            all_results
        )
        ...
        logger.info(
            f"Comprehensive benchmark complete. Total experiments: {experiment_count}"
        )
    ...
    def _calculate_algorithm_stats(
        ...
        for alg_name, alg_results in results["classical_results"].items():
            ...
            tests[f"quantum_vs_{alg_name}"] = {
                "quality_improvement": {
                    ...
                    "improvement_percent": (
                        (
                            statistics.mean(quantum_quality)
                            - statistics.mean(classical_quality)
                        )
                        / statistics.mean(classical_quality)
                        * 100
                        if statistics.mean(classical_quality) > 0
                        else 0
                    ),
                },
                "time_overhead": {
                    ...
                    "overhead_ratio": (
                        ...
                        if statistics.mean(classical_time) > 0
                        else float("inf")
                    ),
                },
            }
    ...
    def export_results(self, results: Dict[str, Any], filename: str) -> None:
        ...
        exportable_results = {
            ...
            "research_metadata": {
                ...
                "algorithms_tested": ["quantum_annealing"]
                + list(results["classical_results"].keys()),
                "total_experiments": sum(
                    len(alg_results)
                    for alg_results in results["classical_results"].values()
                )
                + len(results["quantum_results"]),
            },
        }
