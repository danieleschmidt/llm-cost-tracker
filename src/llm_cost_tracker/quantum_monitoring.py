...
try:
    ...
    warnings.warn(
        "psutil not available, system metrics will be simulated", ImportWarning
    )
import threading
...
class PerformanceMetrics:
    ...
    def update_execution_time(self, execution_time_ms: float) -> None:
        ...
        self.avg_execution_time_ms = self.total_execution_time_ms / max(
...
class SystemHealthMetrics:
    ...
    def is_system_healthy(self) -> bool:
        ...
        return (
            self.task_planner_healthy
            and self.resource_pool_healthy
            and self.scheduler_healthy
            and self.monitoring_healthy
            and self.cpu_usage_percent < 90.0
            and self.memory_usage_percent < 90.0
...
class QuantumTaskMonitor:
    ...
    def __init__(self, planner: QuantumTaskPlanner, monitoring_interval: float = 5.0):
        ...
        self.executor = ThreadPoolExecutor(
            max_workers=2, thread_name_prefix="quantum-monitor"
        )
    ...
    async def start_monitoring(self) -> None:
        ...
        self.monitoring_thread = threading.Thread(
            target=self._monitoring_loop, name="quantum-task-monitor", daemon=True
        )
    ...
    def _collect_system_metrics(self) -> None:
        ...
        try:
            ...
            if len(self.cpu_usage_history) > 0:
                self.performance_metrics.avg_cpu_usage = sum(
                    self.cpu_usage_history
                ) / len(self.cpu_usage_history)
            if len(self.memory_usage_history) > 0:
                self.performance_metrics.avg_memory_usage = sum(
    ...
    def _update_health_status(self) -> None:
        ...
        try:
            ...
            self.health_metrics.resource_pool_healthy = (
                ...
                and pool.allocated_memory <= pool.memory_gb
                and pool.allocated_storage <= pool.storage_gb
                and pool.allocated_bandwidth <= pool.network_bandwidth
            )

            # Check scheduler health (can generate schedules)
            try:
                if len(self.planner.tasks) > 0:
                    test_schedule = self.planner.quantum_anneal_schedule(
                        max_iterations=5
                    )
            ...
            self.health_metrics.health_check_duration_ms = (
    ...
    def _check_alert_conditions(self) -> None:
        ...
        # Memory usage alert
        if (
            self.health_metrics.memory_usage_percent
        ...
        if len(self.execution_times) > 10:
            recent_failures = sum(
                1
                for _ in self.error_history
                if (datetime.now() - _["timestamp"]).total_seconds() < 3600
            )
    ...
    def _record_error(
        self, component: str, error_message: str, severity: str = "error"
    ) -> None:
        ...
        self.health_metrics.error_count_last_hour = sum(
            1 for record in self.error_history if record["timestamp"] > one_hour_ago
        )
    ...
    def record_task_execution(
        ...
        logger.debug(
            f"Recorded execution metrics for task {task_id}: "
            f"{execution_time_ms:.2f}ms, success={success}"
        )

    def record_quantum_event(
        self, event_type: str, task_id: str, details: Dict[str, Any] = None
    ) -> None:
    ...
    def record_scheduling_event(
        self, schedule_quality: float, violations: int, conflicts: int
    ) -> None:
        ...
        self.performance_metrics.avg_schedule_quality = (
            current_avg * (current_count - 1) + schedule_quality
        ) / current_count
        ...
        logger.debug(
            f"Recorded scheduling metrics: quality={schedule_quality:.2f}, "
    ...
    def get_time_series_data(
        ...
        if metric == "cpu_usage":
            ...
            return [
                {
                    "timestamp": (
                        now - timedelta(seconds=i * self.monitoring_interval)
                    ).isoformat(),
                    "value": value,
                }
            ...
            return [
                {
                    "timestamp": (
                        completion_times[i].isoformat()
                        if i < len(completion_times)
                        else now.isoformat()
                    ),
                    "value": value,
                }
...
def console_alert_handler(alert: Dict[str, Any]) -> None:
    ...
    print(
        f"[{timestamp}] QUANTUM ALERT [{alert['severity'].upper()}] {alert['type']}: {alert['message']}"
    )
...
async def create_monitored_planner(
    monitoring_interval: float = 5.0, enable_console_alerts: bool = True
) -> tuple[QuantumTaskPlanner, QuantumTaskMonitor]:
