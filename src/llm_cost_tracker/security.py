...
class AdvancedRateLimiter:
    ...
    def is_allowed(
        self, key: str, user_tier: str = "standard", request_info: Optional[Dict] = None
        ...
        tier_multipliers = {"premium": 3.0, "standard": 1.0, "trial": 0.3}

        tier_multiplier = tier_multipliers.get(user_tier, 1.0)
        adaptive_limit = int(
            self.base_max_requests * tier_multiplier * self.current_load_factor
        )
        security_info["adaptive_limit"] = adaptive_limit

        # Threat detection
        if self.threat_detection and request_info:
            threat_detected, threat_info = self._detect_threats(
                key, current_time, request_info
            )
            if threat_detected:
        ...
        if current_requests >= adaptive_limit:
            ...
            security_info["threat_level"] = (
                "high" if current_requests > adaptive_limit * 2 else "medium"
            )

            logger.warning(
                f"Rate limit exceeded for {key}: {current_requests}/{adaptive_limit}"
            )
            return False, security_info
    ...
    def _detect_threats(
        self, key: str, current_time: float, request_info: Dict
    ) -> Tuple[bool, Dict[str, Any]]:
        ...
        if threat_info["anomaly_score"] >= 4.0:
            ...
            logger.error(
                f"Critical threat detected from {key}: {threat_info['threat_patterns']}"
            )
        elif threat_info["anomaly_score"] >= 2.0:
            threat_info["threat_level"] = "high"
            self.suspicious_ips.add(key)
            logger.warning(
                f"High threat detected from {key}: {threat_info['threat_patterns']}"
            )
    ...
    def _record_violation(self, key: str, current_time: float, violation_type: str):
        ...
        if self.violations[key] >= 5:
            self.suspicious_ips.add(key)
            logger.warning(
                f"Added {key} to suspicious IPs after {self.violations[key]} violations"
            )
        ...
        if self.violations[key] >= 10:
            self.blacklist.add(key)
            logger.error(
                f"Auto-blacklisted {key} after {self.violations[key]} violations"
            )
        ...
        if key in self.last_violation:
            ...
            if time_since_violation > decay_threshold:
                if key in self.violations and self.violations[key] > 0:
                    ...
                    logger.info(
                        f"Decayed violation count for {key} to {self.violations[key]}"
                    )
    ...
    def update_load_factor(
        self, cpu_usage: float, memory_usage: float, queue_length: int = 0
    ):
        ...
        logger.info(
            f"Updated load factor to {self.current_load_factor:.2f} "
    ...
    def get_security_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_violations": {
                key: violations
                for key, violations in self.violations.items()
                if key in self.last_violation
                and current_time - self.last_violation[key] < 3600
            },
        }
