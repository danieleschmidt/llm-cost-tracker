...
class BacklogItem(BaseModel):
    ...
    def calculate_wsjf(self) -> float:
        ...
        if self.effort == 0:
            return 0.0
        base_score = (
            self.value + self.time_criticality + self.risk_reduction
        ) / self.effort
...
class AutonomousBacklogManager:
    ...
    async def score_and_sort_backlog(
        ...
        return sorted(items, key=lambda x: (-x.wsjf_score, x.created_at), reverse=False)

    async def get_next_actionable_item(
        self, items: List[BacklogItem]
    ...
    async def update_metrics(self, items: List[BacklogItem]) -> BacklogMetrics:
        ...
        avg_wsjf = (
            sum(i.wsjf_score for i in active_items) / len(active_items)
            if active_items
            else 0.0
        )
    ...
    async def save_backlog(
        self, items: List[BacklogItem], metrics: BacklogMetrics
    ) -> None:
    ...
    async def generate_status_report(
        ...
        json_report = {
            ...
            "top_priorities": [
                ...
                for i in items[:5]
                if i.status not in ["DONE", "BLOCKED"]
            ],
        }
...
"""
    ...
    async def run_discovery_cycle(self) -> None:
        ...
        new_items = [
            item for item in tech_debt_items + test_items if item.id not in existing_ids
        ]
        ...
        logger.info(
            f"Discovery complete. Found {len(new_items)} new items. "
    ...
    async def execute_macro_loop(self, max_iterations: int = 10) -> None:
        ...
        for iteration in range(max_iterations):
            ...
            logger.info(
                f"Next item to execute: {next_item.title} (WSJF: {next_item.wsjf_score})"
            )
