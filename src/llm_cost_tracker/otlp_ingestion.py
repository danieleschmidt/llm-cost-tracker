...
class OTLPIngestionService:
    ...
    def extract_span_data(self, otlp_span: Dict[str, Any]) -> SpanData:
        ...
        start_time = datetime.fromtimestamp(start_time_ns / 1_000_000_000) if start_time_ns else datetime.now()
        end_time = datetime.fromtimestamp(end_time_ns / 1_000_000_000) if end_time_ns else None
    ...
    def extract_llm_metrics(self, span_data: SpanData) -> Optional[LLMMetricsData]:
        ...
        input_tokens = int(attrs.get('llm.usage.prompt_tokens', 0) or 
                          attrs.get('gen_ai.usage.prompt_tokens', 0))
        output_tokens = int(attrs.get('llm.usage.completion_tokens', 0) or 
                           attrs.get('gen_ai.usage.completion_tokens', 0))
        total_tokens = int(attrs.get('llm.usage.total_tokens', 0) or 
    ...
    async def store_llm_metrics(self, metrics_data: LLMMetricsData) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to store LLM metrics for span {metrics_data.span_id}: {e}")
            return False
...
async def ingest_traces(request: Request) -> Response:
    ...
    try:
        ...
        if 'application/x-protobuf' in content_type:
            ...
            return Response(
                status_code=200,
                content=f"Processed {processed_spans} spans"
            )
