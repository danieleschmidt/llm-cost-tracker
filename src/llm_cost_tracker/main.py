...
from .database import DatabaseConnectionError, DatabaseOperationError, db_manager
...
from .otlp_ingestion import initialize_otlp_service
from .otlp_ingestion import router as otlp_router
from .security import RateLimiter, SecurityHeaders, validate_request_size
from .validation import SecurityValidationError, ValidationError
...
async def security_validation_error_handler(
    ...
    request_id = getattr(request.state, "request_id", "unknown")
    logger.error(
        f"Security validation error: {exc}",
        extra={"request_id": request_id, "security_threat": True},
    )
    return JSONResponse(
        status_code=400,
        content={
...
async def internal_server_error_handler(request: Request, exc: Exception):
    ...
    request_id = getattr(request.state, "request_id", "unknown")
    logger.error(
        f"Internal server error: {exc}", extra={"request_id": request_id}, exc_info=True
    )
    return JSONResponse(
        status_code=500,
        content={
...
from .controllers import budget_router, cost_router, quantum_router, session_router
...
async def cache_metrics() -> JSONResponse:
    ...
    try:
        ...
        return JSONResponse(
            {"cache_stats": stats, "timestamp": datetime.utcnow().isoformat() + "Z"}
        )
    except Exception as e:
...
async def metrics_endpoint() -> JSONResponse:
    ...
    try:
        ...
        return JSONResponse(
            status_code=500, content={"error": "metrics_unavailable", "message": str(e)}
        )
