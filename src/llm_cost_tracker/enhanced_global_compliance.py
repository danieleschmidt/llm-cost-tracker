...
class DataProcessingRecord:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "retention_period": (
                self.retention_period.total_seconds() if self.retention_period else None
            ),
...
class ConsentRecord:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "expiry_timestamp": (
                self.expiry_timestamp.isoformat() if self.expiry_timestamp else None
            ),
        }
...
class EnhancedGlobalCompliance:
        ...
        self.compliance_rules = {
            ComplianceRegulation.GDPR: {
                ...
                "data_retention_limits": {
                    DataCategory.PERSONAL_IDENTIFIER: timedelta(
                        days=365 * 2
                    ),  # 2 years
            ...
            ComplianceRegulation.CCPA: {
                "consent_required_categories": [DataCategory.SENSITIVE_PERSONAL],
                "data_retention_limits": {
                    DataCategory.PERSONAL_IDENTIFIER: timedelta(
                        days=365 * 3
                    ),  # 3 years
            ...
            if compliance_file.exists():
                ...
                for record_data in data.get("processing_records", []):
                    record = DataProcessingRecord(
                        ...
                        retention_period=(
                            timedelta(seconds=record_data["retention_period"])
                            if record_data.get("retention_period")
                            else None
                        ),
                ...
                for consent_data in data.get("consent_records", []):
                    consent = ConsentRecord(
                        ...
                        data_categories=[
                            DataCategory(cat) for cat in consent_data["data_categories"]
                        ],
                        active=consent_data.get("active", True),
                        withdrawal_timestamp=(
                            datetime.fromisoformat(consent_data["withdrawal_timestamp"])
                            if consent_data.get("withdrawal_timestamp")
                            else None
                        ),
                        expiry_timestamp=(
                            datetime.fromisoformat(consent_data["expiry_timestamp"])
                            if consent_data.get("expiry_timestamp")
                            else None
                        ),
                    )
                ...
                logger.info(
                    f"Loaded {len(self.processing_records)} processing records and {len(self.consent_records)} consent records"
                )
    ...
    async def handle_data_subject_request(
        ...
        logger.info(
            f"Processing {request_type.value} request for subject {data_subject_id}"
        )
        ...
        try:
            if request_type == DataSubjectRight.ACCESS:
                ...
                if additional_data:
                    updated_count = await self._update_subject_data(
                        data_subject_id, additional_data
                    )
                ...
                # Restrict processing
                restricted_count = await self._restrict_subject_processing(
                    data_subject_id
                )
                ...
                objection_count = await self._handle_processing_objection(
                    data_subject_id
                )
    ...
    async def _get_subject_data(self, data_subject_id: str) -> Dict[str, Any]:
        ...
        subject_data = {"processing_records": [], "consent_records": []}
    ...
    async def _update_subject_data(
        self, data_subject_id: str, update_data: Dict[str, Any]
    ) -> int:
    ...
    async def _handle_processing_objection(self, data_subject_id: str) -> int:
        ...
        for record in self.processing_records:
            if (
                record.data_subject_id == data_subject_id
                and record.legal_basis == ProcessingLegalBasis.LEGITIMATE_INTERESTS
            ):
                objection_count += 1
    ...
    def _check_processing_compliance(self, record: DataProcessingRecord) -> None:
        ...
        for regulation, rules in self.compliance_rules.items():
            ...
            if any(
                cat in consent_required_categories for cat in record.data_categories
            ):
                if (
                    record.legal_basis == ProcessingLegalBasis.CONSENT
                    and not record.consent_id
                ):
                    self._create_alert(
                        regulation,
                        "high",
                    ...
                    record.consent_id and record.consent_id not in self.consent_records
                ):
                    self._create_alert(
                        regulation,
                        "high",
                    ...
                    record.consent_id
                    and not self.consent_records[record.consent_id].active
                ):
                    self._create_alert(
                        regulation,
                        "critical",
                ...
                for regulation, rules in self.compliance_rules.items():
                    ...
                    for record in self.processing_records:
                        if record.retention_period:
                            retention_deadline = (
                                record.timestamp + record.retention_period
                            )
                        else:
                            ...
                            if default_retention:
                                retention_deadline = (
                                    record.timestamp + default_retention
                                )
                            else:
                                continue
                ...
                for consent in self.consent_records.values():
                    if consent.active and consent.expiry_timestamp:
                        ...
                        if (
                            ...
                            and current_time < consent.expiry_timestamp
                        ):
                            self._create_alert(
                                ComplianceRegulation.GDPR,
                                "low",
    ...
    async def save_compliance_records(self) -> None:
        ...
        try:
            compliance_data = {
                ...
                "compliance_alerts": [
                    alert.to_dict() for alert in self.compliance_alerts
                ],
            }
...
async def main():
    ...
    access_result = await compliance_system.handle_data_subject_request(
        data_subject_id="user_123", request_type=DataSubjectRight.ACCESS
    )
    ...
    print(
        f"   • Processing Records: {dashboard_data['summary']['total_processing_records']}"
    )
