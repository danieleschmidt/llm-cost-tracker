...
async def get_model_recommendations(
    task_type: Optional[str] = Query(
        None, description="Type of task (e.g., 'analysis', 'generation')"
    ),
    budget_constraint: Optional[float] = Query(
        None, ge=0, description="Maximum cost per token"
    ),
    performance_requirement: str = Query(
        "balanced",
        description="Performance requirement: 'cost', 'performance', or 'balanced'",
    ),
...
async def route_model_request(
    ...
    try:
        routing = await routing_service.route_model_request(
            original_model, user_id, context
        )
...
async def update_model_pricing(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=400, detail="Failed to update model pricing"
            )
...
async def get_efficiency_summary(
    ...
    try:
        ...
        avg_efficiency_score = sum(model["efficiency_score"] for model in models) / len(
            models
        )
        ...
        if worst_model["efficiency_score"] < 50:
            recommendations.append(
                f"Consider replacing {worst_model['model_name']} with more efficient alternatives"
            )
