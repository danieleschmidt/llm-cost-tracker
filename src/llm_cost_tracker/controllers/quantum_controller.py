...
async def create_task(task_request: TaskRequest) -> TaskResponse:
    ...
    try:
        if task_request.id in quantum_planner.tasks:
            raise HTTPException(status_code=400, detail=f"Task {task_request.id} already exists")
...
async def delete_task(task_id: str) -> Dict[str, str]:
    ...
    try:
        ...
        if task.state == TaskState.EXECUTING:
            raise HTTPException(status_code=400, detail=f"Cannot delete executing task {task_id}")
...
async def get_optimal_schedule() -> ScheduleResponse:
    ...
    try:
        ...
        for i, task_id in enumerate(schedule):
            task = quantum_planner.tasks[task_id]
            total_duration += task.estimated_duration.total_seconds() / 60.0  # Convert to minutes
...
async def execute_tasks(
    ...
    try:
        ...
        logger.info(f"Starting quantum task execution with {len(quantum_planner.tasks)} tasks")
