...
def validate_json_object(
    value: Any, max_depth: int = 10, max_keys: int = 100, field_name: str = "field"
) -> Dict[str, Any]:
    """
    ...
    def check_depth(obj, current_depth=0):
        ...
        if isinstance(obj, dict):
            if len(obj) > max_keys:
                raise ValidationError(
                    f"{field_name} has too many keys (max {max_keys})"
                )
            ...
            if len(obj) > max_keys:  # Apply same limit to arrays
                raise ValidationError(
                    f"{field_name} array too large (max {max_keys} items)"
                )
    ...
    def validate_attributes_dict(cls, v):
        if v is not None:
            return validate_json_object(
                v, max_depth=5, max_keys=50, field_name="attributes"
            )
        return {}
...
class LLMMetricsValidator(BaseModel):
    ...
    def validate_metadata_dict(cls, v):
        if v is not None:
            return validate_json_object(
                v, max_depth=3, max_keys=20, field_name="metadata"
            )
        return {}
