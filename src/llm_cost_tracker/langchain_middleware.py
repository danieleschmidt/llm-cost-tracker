...
class LLMCostTracker(AsyncCallbackHandler):
    ...
    def __init__(
        self, application_name: str = "default", user_id: Optional[str] = None
    ):
        super().__init__()
    ...
    def _setup_otel(self) -> None:
        ...
        otlp_exporter = OTLPSpanExporter(
            endpoint=get_settings().otel_endpoint, insecure=True
        )
    ...
    def _extract_model_name(self, serialized: Dict[str, Any]) -> str:
        ...
        if "kwargs" in serialized:
            model = serialized["kwargs"].get("model_name") or serialized["kwargs"].get(
                "model"
            )
            if model:
                return model
    ...
    def _calculate_costs(
        ...
        costs = self.model_costs.get(
            model_name.lower(), {"input": 0.001, "output": 0.002}
        )
...
def create_cost_tracker(
    application_name: str = "default", user_id: Optional[str] = None
) -> LLMCostTracker:
