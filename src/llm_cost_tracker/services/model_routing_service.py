...
class ModelRoutingService:
    ...
    async def route_model_request(
        ...
        try:
            ...
            alternative = await self._find_optimal_alternative(
                original_model, budget_status.get("budget_remaining", 0), context
            )
            ...
            savings = await self._calculate_cost_savings(
                original_model, alternative["model"]
            )
        ...
        try:
            ...
            for row in result:
                model_data = {
                    ...
                    "cost_share_percentage": round(
                        (float(row["total_cost"]) / total_cost_all_models) * 100, 2
                    ),
                    "slow_request_rate": round(
                        (row["slow_requests"] / row["usage_count"]) * 100, 2
                    ),
                    "tier": self._get_model_tier(row["model_name"]),
                    "efficiency_rating": self._rate_model_efficiency(
                        float(row["avg_cost"]), float(row["avg_latency"])
                    ),
                }
            ...
            budget_optimization_rate = (
                (budget_models_usage / total_usage) * 100 if total_usage > 0 else 0
            )
    ...
    async def _find_optimal_alternative(
        self,
        original_model: str,
        ...
        try:
                    ...
                    candidates.append(
                        {
                            ...
                            "cost_reduction": (
                                (original_pricing["input"] - pricing["input"])
                                / original_pricing["input"]
                            )
                            * 100,
            ...
            return {
                ...
                "performance_impact": self._estimate_performance_impact(
                    original_tier, best_candidate["tier"]
                ),
    ...
    async def _calculate_cost_savings(
        self, original_model: str, alternative_model: str
    ) -> Dict:
        ...
        try:
            ...
            input_savings = (
                (original_pricing["input"] - alternative_pricing["input"])
                / original_pricing["input"]
            ) * 100
            output_savings = (
                (original_pricing["output"] - alternative_pricing["output"])
                / original_pricing["output"]
            ) * 100
    ...
    def _calculate_recommendation_score(
        self, pricing: Dict, performance: Dict, requirement: str
    ) -> float:
    ...
    def _score_alternative_model(
        self, candidate: Dict, context: Optional[Dict] = None
    ) -> float:
    ...
    def _estimate_performance_impact(
        self, original_tier: str, alternative_tier: str
    ) -> str:
    ...
    def _generate_routing_recommendations(
        self, routing_analysis: List[Dict]
    ) -> List[str]:
        ...
        if premium_usage / total_usage > 0.3:
            recommendations.append(
                "High premium model usage detected - consider optimizing for cost"
            )

        # Check for performance issues
        slow_models = [
            model for model in routing_analysis if model["slow_request_rate"] > 20
        ]
        if slow_models:
            recommendations.append(
                f"Performance issues detected in {len(slow_models)} models"
            )

        # Check for efficiency
        inefficient_models = [
            model for model in routing_analysis if model["efficiency_rating"] == "poor"
        ]
        if inefficient_models:
            recommendations.append(
                "Consider replacing inefficient models with alternatives"
            )
