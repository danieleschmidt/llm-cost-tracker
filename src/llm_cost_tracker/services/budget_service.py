...
class BudgetService:
    ...
    async def update_budget_rule(
        self, rule_id: int, updates: BudgetRuleUpdate
    ) -> Optional[BudgetRule]:
        ...
        try:
            ...
            query = (
                f"UPDATE budget_rules SET {set_clause} WHERE id = %(id)s RETURNING *"
            )
    ...
    async def check_budget_violations(
        ...
        try:
            ...
            for rule_data in rules:
                ...
                current_spend = await self._calculate_current_month_spend(
                    rule.id, user_id
                )
                    ...
                    violations.append(
                        {
                            ...
                            "overage_amount": float(
                                rule.current_spend_usd - rule.monthly_limit_usd
                            ),
                    ...
                    violations.append(
                        {
                            ...
                            "threshold_amount": float(
                                rule.monthly_limit_usd * rule.alert_threshold
                            ),
                            "budget_limit": float(rule.monthly_limit_usd),
    ...
    async def predict_monthly_cost(self, rule_id: int) -> Dict:
        ...
        try:
            ...
            days_in_month = (
                month_start.replace(month=month_start.month + 1) - month_start
            ).days
            predicted_cost = avg_daily_cost * days_in_month

            # Calculate confidence based on variance
            variance = (
                sum((cost - avg_daily_cost) ** 2 for cost in daily_costs) / days_elapsed
            )
    ...
    async def _calculate_current_month_spend(
        self, rule_id: int, user_id: Optional[str] = None
    ) -> Decimal:
    ...
    async def _update_rule_current_spend(self, rule_id: int, current_spend: Decimal):
        ...
        try:
            ...
            await self.db.execute_query(
                query, [current_spend, datetime.utcnow(), rule_id]
            )
