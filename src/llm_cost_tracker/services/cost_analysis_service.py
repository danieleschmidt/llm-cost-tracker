...
try:
    ...
    class NumpyFallback:
        ...
        def polyfit(self, x, y, deg):
            ...
            x_mean = sum(x) / n
            y_mean = sum(y) / n
            slope = sum((x[i] - x_mean) * (y[i] - y_mean) for i in range(n)) / max(
                1, sum((x[i] - x_mean) ** 2 for i in range(n))
            )
...
class CostAnalysisService:
                ...
                models.append(
                    {
                        ...
                        "avg_cost_per_request": round(
                            float(row["avg_cost_per_request"] or 0), 6
                        ),
                        "cost_per_token": round(float(row["cost_per_token"] or 0), 8),
                        "tokens_per_second": round(
                            float(row["tokens_per_second"] or 0), 2
                        ),
                ...
                opportunities.append(
                    {
                        ...
                        "recommendations": prompt_opportunities.get(
                            "recommendations", []
                        ),
                    }
                )
    ...
    async def calculate_roi_metrics(self, days: int = 30) -> Dict:
        ...
        try:
            ...
            estimated_human_hours_saved = (
                total_requests * 0.25
            )  # Assume 15 min saved per request
            estimated_hourly_rate = 50.0  # $50/hour average
            estimated_value_generated = (
                estimated_human_hours_saved * estimated_hourly_rate
            )

            # Calculate ROI
            roi_percentage = (
                (estimated_value_generated - total_cost) / max(total_cost, 1)
            ) * 100
            ...
            return {
                ...
                "efficiency_rating": (
                    ...
                    else "medium" if roi_percentage > 50 else "low"
                ),
            }
                ...
                anomalies.append(
                    {
                        ...
                        "severity": (
                            "high" if abs(cost - mean_cost) > 3 * std_cost else "medium"
                        ),
                    }
                )
    ...
    async def _generate_cost_recommendations(
        self, daily_data: List[Dict], trend: str
    ) -> List[str]:
        ...
        if trend == "increasing":
            recommendations.append(
                "Cost trend is increasing - consider implementing budget alerts"
            )
        ...
        if len(costs) > 1:
            ...
            if cv > 0.5:
                recommendations.append(
                    "High cost variability detected - investigate usage patterns"
                )

        # Check for high latency costs
        avg_latencies = [
            float(row["avg_latency"]) for row in daily_data if row["avg_latency"]
        ]
        if avg_latencies and np.mean(avg_latencies) > 5000:
            recommendations.append(
                "High average latency detected - consider model optimization"
            )
    ...
    async def _analyze_peak_usage_patterns(self) -> Dict:
        ...
        try:
            ...
            hourly_data = {
                int(row["hour"]): float(row["hourly_cost"]) for row in result
            }
            ...
            peak_hours = [
                hour for hour, cost in hourly_data.items() if cost > max_cost * 0.8
            ]
    ...
    def _calculate_efficiency_score(
        self, cost_per_token: float, avg_latency: float, tokens_per_second: float
    ) -> float:
