...
class SessionService:
    ...
    async def start_session(
        self, user_id: str, application_name: Optional[str] = None
    ) -> str:
    ...
    async def update_session(self, session_id: str, usage_data: Dict) -> bool:
        ...
        try:
            ...
            if session["total_requests"] % 10 == 0 or session[
                "total_cost_usd"
            ] > Decimal("1.00"):
    ...
    async def end_session(self, session_id: str) -> Optional[UserSession]:
        ...
        try:
            ...
            logger.info(
                f"Ended session {session_id} - Duration: {user_session.session_duration_minutes:.1f}min, "
    ...
    async def get_session_metrics(
        ...
        try:
            ...
            most_used_models = (
                [row["model_name"] for row in models_result] if models_result else []
            )

            # Calculate peak concurrent sessions (approximate)
            peak_concurrent = await self._calculate_peak_concurrent_sessions(
    ...
    async def analyze_user_behavior(self, user_id: str, days: int = 30) -> Dict:
        ...
        try:
            ...
            avg_session_length = sum(
                float(session["duration_minutes"]) for session in sessions
            ) / len(sessions)
    ...
    async def get_session_cost_breakdown(self, session_id: str) -> Dict:
        ...
        try:
            ...
            usage_data = await self.db.execute_query(
            ...
            return {
                ...
                "cost_per_minute": round(
                    float(session["total_cost_usd"])
                    / max(1, (end_time - start_time).total_seconds() / 60),
                    6,
                ),
    ...
    async def _calculate_peak_concurrent_sessions(
        ...
        try:
            ...
            while current_time <= end_date:
                where_conditions = ["start_time <= %s", "last_activity >= %s"]
    ...
    def _analyze_session_patterns(self, sessions: List[Dict]) -> Dict:
        ...
        peak_hour = (
            max(set(session_hours), key=session_hours.count) if session_hours else 0
        )
        ...
        return {
            ...
            / 30,  # sessions per day (assuming 30-day period)
            "cost_consistency": (
                round(np.std(costs) / max(avg_cost, 0.001), 2) if len(costs) > 1 else 0
            ),
        }
    ...
    def _calculate_user_cost_efficiency(self, sessions: List[Dict]) -> Dict:
        ...
        rating = (
            ...
            if efficiency_score > 80
                ...
                else "fair" if efficiency_score > 40 else "poor"
            )
        )
    ...
    def _generate_user_recommendations(
        self, sessions: List[Dict], patterns: Dict
    ) -> List[str]:
        ...
        if avg_cost > 5:
            recommendations.append(
                "High session costs detected - consider using more cost-effective models"
            )

        # Duration-based recommendations
        if avg_duration > 60:
            recommendations.append(
                "Long sessions detected - consider breaking work into shorter sessions"
            )
        elif avg_duration < 2:
            recommendations.append(
                "Very short sessions - consider batching requests for better efficiency"
            )
        ...
        if frequency > 10:
            recommendations.append(
                "High usage frequency - consider setting up budget alerts"
            )
        ...
        if consistency > 1:
            recommendations.append(
                "High cost variability - review usage patterns for optimization"
            )

        if not recommendations:
            recommendations.append(
                "Usage patterns look efficient - keep up the good work!"
            )

        return recommendations

    def _calculate_session_efficiency(
        self, session: Dict, model_breakdown: List[Dict]
    ) -> float:
