...
class MetricsCollector:
    ...
    async def collect_metrics(self) -> ResourceMetrics:
        ...
        disk_io_percent = min(100, (disk_io.read_bytes + disk_io.write_bytes) / (1024 * 1024 * 100))  # MB/s
        network_io_percent = min(100, (network_io.bytes_sent + network_io.bytes_recv) / (1024 * 1024 * 10))  # MB/s
...
class AutoScaler:
    ...
    async def evaluate_scaling(self) -> List[ScalingEvent]:
        ...
        events = []
        avg_metrics = self.metrics_collector.get_average_metrics(seconds=120)  # 2 minute average
    ...
    def _evaluate_rule(self, rule: ScalingRule, metric_value: float) -> ScalingDirection:
    ...
    async def _execute_scaling(
        ...
        try:
            ...
            if rule.resource_type in self.resource_handlers:
                ...
                logger.info(f"Scaled {rule.resource_type.value} from {current_value} to {new_value}")
            else:
