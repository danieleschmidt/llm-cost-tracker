...
class AnomalyDetector:
    ...
    def add_metric(self, metric: MetricPoint):
        ...
        now = datetime.now()
        should_update = (
            len(self.metric_history[metric_key]) >= 20 and
            (metric_key not in self.last_baseline_update or 
             now - self.last_baseline_update[metric_key] > timedelta(minutes=10))
        )
    ...
    def _update_baseline(self, metric_key: str):
        ...
        logger.debug(f"Updated baseline for {metric_key}: mean={mean:.2f}, stdev={stdev:.2f}")
    ...
    def detect_anomaly(self, metric: MetricPoint) -> Optional[Dict[str, Any]]:
        ...
        if value > baseline['upper_threshold']:
            ...
            deviation = (value - baseline['mean']) / baseline['stdev'] if baseline['stdev'] > 0 else 0
            ...
            deviation = (baseline['mean'] - value) / baseline['stdev'] if baseline['stdev'] > 0 else 0
            anomaly_info['severity_score'] += min(10.0, deviation)
        ...
        confidence = min(1.0, baseline['sample_count'] / 50.0)  # Higher confidence with more samples
        if baseline['stdev'] > 0:
            stability_factor = min(1.0, baseline['mean'] / baseline['stdev'])  # More stable = higher confidence
...
class EnhancedMonitoringSystem:
    ...
    async def _send_email_notification(self, alert: Alert, channel_config: Dict):
        ...
        logger.info(f"Email notification (simulated): {alert.name} to {channel_config.get('recipients')}")
    ...
    def _update_sla_metrics(self, alert: Alert):
        ...
        if alert.severity in [AlertSeverity.ERROR, AlertSeverity.CRITICAL]:
            if alert.metric_name in ['response_time_ms', 'error_rate_percent']:
                sla_key = 'response_time' if 'response_time' in alert.metric_name else 'error_rate'
    ...
    def _calculate_sla_metrics(self):
        ...
        recent_errors = [
            m for m in self.metrics_buffer[-100:] 
    ...
    def _cleanup_old_data(self):
        ...
        for sla_name, sla_data in self.sla_metrics.items():
            sla_data['violations'] = [
                violation for violation in sla_data['violations']
